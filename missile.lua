...
Shortcomings:
  - Want a way to restrict this to affect just a single weapon group, then you
      can have one per missile type and drop the whole high/low thing.
  - Assumes the role of a missile is defined by its current height over sea
      level; must ensure air/surface missiles are catapulted above the water
      before the guidance will take effect. (Would be nice to get design info.)
...
-- Tunables (high = SAM/SSM/AGM; low = torpedo) --------------------------------
...
high_prox_abort         = 3
low_prox_abort          = 3
-- High missiles will avoid going below this height until they are close enough
...
-- TODO only used for focusing missiles; make this a 'blinkers' tunable
half_pi = math.pi * 0.5
...
function AngleToTarget(
  I, missile_info, missile_high, target, target_in_missile_coords)
...
function TimeToTarget(
  ...
  if missile_high and missile_speed < high_speed_estimate
                 then missile_speed = high_speed_estimate end
  if missile_low  and missile_speed <  low_speed_estimate
                 then missile_speed =  low_speed_estimate end
  ...
  local turn_rate = low_turn_rate
  if missile_high then turn_rate = high_turn_rate end
...
function MissileCanHit(I, missile_info, missile_high, target)
  local maximum_range = low_maximum_range
  local turn_rate     = low_turn_rate
  if missile_high then
    maximum_range = high_maximum_range
    turn_rate     = high_turn_rate
  end
...
function BestTargetForMissile(I, missile_info, missile_high, targets)
  ...
  table.sort(targets, function(a,b)
    ...
    if a_angle > half_pi then a_angle = half_pi end
    if b_angle > half_pi then b_angle = half_pi end
  ...
  for ignore, target in ipairs(targets) do
    if MissileCanHit(I, missile_info, missile_high, target) then
      return target
    end
  end
...
function Update(I)
  -- Target locations
  local high_targets = {} -- for air/surface missiles
  local low_targets  = {} -- for torpedos
  ...
  for mainframe = 0, mainframes-1 do
    ...
    for target = 0, targets-1 do
      ...
      if target_info.Valid and target_position_info.Valid then
        if target_position_info.AltitudeAboveSeaLevel > -sea_crossover_tolerance
          then table.insert(high_targets, target_info) end
        if target_position_info.AltitudeAboveSeaLevel <  sea_crossover_tolerance
          then table.insert( low_targets, target_info) end
      end
    end
  end
  ...
  local transcievers = I:GetLuaTransceiverCount()
  for transciever = 0, transcievers-1 do
    ...
    for missile = 0, missiles-1 do
      ...
      if measurement_mode then -- suspend normal behaviour
        ...
        local missile_high = false
        local targets = low_targets
        if missile_info.Position.y >= 0 then
          missile_high = true
          targets = high_targets
        end
        local best_target = BestTargetForMissile(
          I, missile_info, missile_high, targets)

        if best_target == nil then
          -- Nothing we can hit!
          if missile_high and missile_info.TimeSinceLaunch < max_climb_age then
          ...
          local prox_abort = low_prox_abort
          if missile_high then prox_abort = high_prox_abort end
          ...
          -- moment
          if missile_high and aim_at.y < 0 then
            local angle_to_target = AngleToTarget(
              I, missile_info, missile_high, target, target_in_missile_coords)
            local time_to_turn = TimeToTarget(
              I, missile_info, missile_high, target, target_in_missile_coords)
              * 0.5 -- get ready early so we don't abort
            if angle_to_target < high_turn_rate * time_to_turn then
